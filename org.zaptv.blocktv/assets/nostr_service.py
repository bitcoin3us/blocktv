...
class NostrManager:
    ...
    def configure_relays(self, relays):
        """Add default relay(s) without configuring an identity.

        For read-only use cases (e.g. watching public zap receipts) that
        need relay connections but have no nsec. Unlike
        configure_identity, this publishes nothing (no NIP-65 relay list).

        relays may be a single URL string (comma-separated ok) or a list."""
        normalised = _normalize_relays(relays)
        added = False
        for url in normalised:
            if url not in self._default_relays:
                self._default_relays.append(url)
                added = True
        self._configured_relays = list(self._default_relays)
        if added:
            self._relays_dirty = True
        self._ensure_main_task()
